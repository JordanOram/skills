---
name: "solomon-browser-debugging"
description: "Debug product issues from the user interface back to code, logs, data, and deployment state."
...
# Solomon Browser Debugging

## When to use

Use when something looks wrong, breaks in browser, throws errors, fails to load, has visual issues, or needs UI-to-code diagnosis.
...
1. Reproduce the issue first. Do not guess.
2. Check browser console, network, route state, server logs, and recent changes.
3. Classify the bug: UI, data, auth, integration, styling, deployment, or environment.
4. Trace the shortest path from visible symptom to likely source.
5. Make the smallest safe fix when implementation is requested.
6. Verify the exact user flow again after the fix.
7. Report root cause, fix, and remaining risk.
